...
def one_train_gpytorch_model(    
    ...
    mll = DeepApproximateMLL(VariationalELBO(gp_model.likelihood, gp_model, x_train.shape[-2]))
...
def experiment_run(
    ...
    **kwargs,
    ...
    for i in range(data.y_train.shape[1]):
        ...
        y_pred = one_DGP_gpytorch_inference(
            gp_model, x_test=x_test
        )
...
config.exp_name = "DGP12_DeepGP_Far0"
