...
import numpy as np
from upjab_ActGP.models.gp_design.gp_model_from_config_module_04 import get_gp_model
...
config.exp_name = "DGP22_ActGP_ChooseEff_Far2"
...
def get_kth_largest_index(arr, k):
    """
    Finds the index of the k-th largest element in a NumPy array.
    ...
    Returns:
        int: The index of the k-th largest element.
    """
    if not (1 <= k <= len(arr)):
        raise ValueError("k must be within the valid range of array length.")

    # argpartition finds the index that would put the k-th largest element
    # in its sorted position. We use -k because we are looking for largest.
    # The result is an array of indices, and the index we want is at the -k-th position.
    partitioned_indices = np.argpartition(arr, -k)
    
    # The element at partitioned_indices[-k] is the k-th largest value.
    # We return this index.
    return partitioned_indices[-k]
...
while True:
    ...
    model = get_gp_model()
    model.fit(x_training, y_training[:, 3])        
    ...
    if x_len == 900:
        ...
        # 3
        # acquisition_value = y_mean +  beta * y_std
        # index_list = np.argpartition(acquisition_value, -TOP_K)[-TOP_K:]
        ...
        # 5
        # acquisition_value = y_mean +  beta * y_std
        # len_x_eval = len(x_evaluate)
        # top_Nth = int(len_x_eval * top_region)
        # indx = get_kth_largest_index(acquisition_value, top_Nth)
        # index_list = np.array([indx])

        # 6
        # len_x_eval = len(x_evaluate)
        # top_Nth = int(len_x_eval * top_region)
        # indx = get_kth_largest_index(y_std, top_Nth)
        # index_list = np.array([indx])

        # 7
        # len_x_eval = len(x_evaluate)
        # top_Nth = int(len_x_eval * top_region)
        # top_region_index_list = np.argpartition(y_std, -top_Nth)[-top_Nth:]
        # index_list = np.random.choice(top_region_index_list, TOP_K, replace=False)
        # print('break')
