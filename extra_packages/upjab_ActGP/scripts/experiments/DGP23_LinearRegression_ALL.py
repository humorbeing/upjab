...
from upjab_ActGP.models.gp_design.gpytorch_ExactGP_v01 import Get_GP_model


from upjab_ActGP.train_val_test.utils import DIVIDE

from upjab_ActGP.train_val_test.one_LR_inference_module import (
    one_LR_inference,
)
# from efficientGP.models.gp_design.gp_model_from_config_module_02 import get_gp_model
...
def LinearRegressionModels_exp(
    get_LR_model            
):
    def experiment_run(
        num_trainval=900,
    ...
    def log_results(
        ...
        log_path = f"{log_folder}/linear_regreesion_models_far0/{exp_name}/{num_trainval}/{random_round_num}/" + "{time}_" + f"{exp_name}.log"
    ...
    config.exp_name = f"DGP00_SciLR_{get_LR_model.__name__}_Far0"
...
for get_LR_model in get_LR_model_list:
    LinearRegressionModels_exp(
        get_LR_model=get_LR_model
    )
