...
from upjab_ActGP.models.gp_design.gpytorch_ApproximateGP_v01 import Get_GP_model
from upjab_ActGP.train_val_test.one_gpytorch_inference_module import (
    one_gpytorch_inference,
)
...
def one_train_gpytorch_model(    
    ...
    gp_model = Get_GP_model(
        num_inducing_point=num_inducing_point, x_train=x_train
    )
    gp_model = gp_model.to(device)

    optimizer = torch.optim.Adam(
        [
            {"params": gp_model.model.parameters()},
    ...
    mll = gpytorch.mlls.VariationalELBO(
        gp_model.likelihood, gp_model.model, num_data=y_train.size(0)
    )
    ...
    **kwargs,
    ...
    for i in range(data.y_train.shape[1]):
        gp_model = one_train_gpytorch_model(            
            ...
            data.x_val,
            data.y_val[:, i],
            num_inducing_point=num_inducing_point,
            EPOCH=EPOCH,
        )
...
config.exp_name = "DGP11_ApproGP_Far0"
