...
class LoadDATA:
    def __init__(self,        
        ...
        if num_Feature_Repeat > 1:
            # print(f'Feature Repeat: {num_Feature_Repeat}')
            x_trainval_temmp = x_trainval.copy()
            x_train_temmp = x_train.copy()
            x_val_temmp = x_val.copy()
            x_test_temmp = x_test.copy()
            for _ in range(num_Feature_Repeat - 1):
                x_trainval = np.hstack((x_trainval, x_trainval_temmp))
                x_train = np.hstack((x_train, x_train_temmp))
                x_val = np.hstack((x_val, x_val_temmp))
                x_test = np.hstack((x_test, x_test_temmp))
