...
path_list = [
    ['logs/e10_eval_return.npy', 'epsilon 10%'],
    ['logs/f1_eval_return.npy', 'Line 0.9-0.05'],
    ['logs/f2_eval_return.npy', 'J-curve 0.99-0.05'],    
    ['logs/f4_eval_return.npy', 'S-curve 0.6-0.05'],
    ['logs/f7_eval_return.npy', 'Line 0.3-0.01'],
    ['logs/f6_eval_return.npy', 'J-curve 0.5-0.01'],    
    ['logs/f8_eval_return.npy', 'S-curve 0.2-0.01'],    
    ['logs/f11_eval_return.npy', 'J-curve 0.15-0.01'],
    ['logs/e01_eval_return.npy', 'epsilon 1%'],
    ['logs/e05_eval_return.npy', 'epsilon 5%'],
    ['logs/e15_eval_return.npy', 'epsilon 15%'],
    ['logs/e20_eval_return.npy', 'epsilon 20%'],
...
limit_true_list = []
for i in np_list:
    limit_true_list.append(i==-999)


limit_num_list = []
for i in limit_true_list:
    limit_num_list.append(i.sum(0)/1000)
    # print('')


zero_list = []
for i, old_list in enumerate(np_list):
    temp11 = np.copy(old_list)
    temp11[limit_true_list[i]] = 0
    zero_list.append(temp11)
    # print("")
...
zero_mean_list = []
for i in zero_list:
    zero_mean_list.append(np.mean(i, 0))
...
# for i, s in enumerate(zero_mean_list):
#     plt.plot(x, s, label=path_list[i][1])

for i, s in enumerate(limit_num_list):
    plt.plot(x, s, label=path_list[i][1])
...
plt.legend()
plt.title('Percentage of Fail-to-Complete in Episode (On Evaluation Policy)')
plt.ylim((0,1))
# plt.plot(x, t_median)
# plt.plot(x, e_median)
# plt.plot(x, f_median)
