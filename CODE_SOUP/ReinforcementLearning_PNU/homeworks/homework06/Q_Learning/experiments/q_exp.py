...
import logging
logging.basicConfig(filename='logs/test.log', level=logging.INFO, filemode='w')  # filemode='w'
from tqdm import tqdm
import time
if __name__ == "__main__":
    lr_list0 = np.linspace(0.0001, 0.001, 5)
    lr_list1 = np.linspace(0.001, 0.01, 5)
    lr_list2 = np.linspace(0.01, 0.1, 5)
    lr_list3 = np.linspace(0.1, 1.0, 5)    
    lr_list = np.concatenate((lr_list0[1:], lr_list1[1:], lr_list2[1:], lr_list3[1:]))
    env = Env()
    env.is_render = False
    for lr in lr_list:
